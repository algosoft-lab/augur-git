//! Application library shared by the `augur-git` and `augurgit` binaries.
//!
//! Both binaries are thin entry points around [`run`], which parses the
//! command line, forwards open requests to a running instance when one
//! exists, and otherwise boots the GPUI application as the primary instance.
...
/// Process entry shared by both binaries.
pub fn run() {
    configure_windows_error_mode();
    logging::init();
    log::info!("[app] starting augur-git");

    let invocation = crate::core::cli::parse_from_env();
    if crate::core::ipc::forward_to_running_instance(&invocation.paths) {
        return;
    }
    let instance_server = crate::core::ipc::InstanceServer::start();

    let app = gpui_platform::application().with_assets(AppAssets);
    workspace::run(
        app,
        workspace::remote_open::PendingOpen {
            startup_paths: invocation.paths,
            instance_server,
        },
    );
}
