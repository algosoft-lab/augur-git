...
use crate::core::extension::{ExtensionError, ExtensionRunRecord, MAX_EXTENSION_RUN_HISTORY};
...
fn history_path(extension_id: &str) -> Result<PathBuf, ExtensionError> {
    ...
    let data_root = root
        ...
        .ok_or_else(|| ExtensionError::Io("invalid extension data path".into()))?;
    Ok(data_root.join(format!("{extension_id}-history.json")))
}

fn load_run_history(extension_id: &str) -> Result<Vec<ExtensionRunRecord>, ExtensionError> {
    let path = history_path(extension_id)?;
    match fs::read_to_string(path) {
        Ok(text) => serde_json::from_str(&text)
            .map_err(|error| ExtensionError::Io(format!("invalid extension run history: {error}"))),
...
pub fn append_run_history(
    ...
    let text = serde_json::to_string_pretty(&history).map_err(|error| {
        ExtensionError::Io(format!("failed to encode extension history: {error}"))
    })?;
    let temp = path.with_extension(format!("json.tmp-{}", std::process::id()));
    fs::write(&temp, text).map_err(|error| {
        ExtensionError::Io(format!("failed to write extension history: {error}"))
    })?;
    fs::rename(&temp, &path).map_err(|error| {
        let _ = fs::remove_file(&temp);
        ExtensionError::Io(format!("failed to replace extension history: {error}"))
    })
}
