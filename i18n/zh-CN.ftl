...
shortcut-edit-description = 输入类似 cmd-q 或 alt-f4 的按键组合，多个组合用英文逗号分隔。按回车应用；留空表示解绑。
shortcut-app-quit = 退出应用
shortcut-default-hint = 默认：{ $keys }
shortcut-invalid-combo = 无效的按键组合：{ $combo }
shortcut-reset = 重置
language-title = 界面语言
...
agent-add-custom-profile = 自定义配置…
agent-remove = 移除
agent-default-badge = 默认
agent-empty-title = 尚未配置任何 Agent
agent-empty-description = 在添加 Agent 之前，AI 提交、合并和变基保持禁用。内置预设可一键添加，且只有在添加后才会被检测。
agent-launch-settings-toggle = 启动设置
agent-card-no-args = 无固定参数
...
agent-prompt-window-title = Agent 任务
agent-prompt-status-running = Agent 正在处理任务
agent-prompt-status-completed = Agent 已报告完成
agent-prompt-status-unverified = Agent 已退出且未报告完成
agent-prompt-stop = 停止任务
