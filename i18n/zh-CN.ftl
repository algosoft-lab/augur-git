...
toolbar-branch = 分支
toolbar-compare = 比较
toolbar-agent-task = 新建 Agent 任务
toolbar-refresh = 刷新
toolbar-settings = 设置
toolbar-busy = 操作中…

# ===== 外部 Agent 会话 =====
agent-task-title = 新建 Agent 任务
agent-task-placeholder = 描述要交给编码 Agent 的任务…
agent-task-empty = 请先输入任务内容，再启动 Agent。
agent-profile-label = Agent
agent-context-none = 未选择审查对象
agent-review-tab = 审查
agent-new-task = 新建 Agent 任务
agent-start = 启动 Agent
agent-cancel = 取消
agent-profile-invalid = 当前 Agent 配置无效。
agent-start-failed = 无法启动 Agent：{ $error }
agent-status-starting = 启动中
agent-status-running = 运行中
agent-status-exited = 已退出（{ $code }）
agent-status-exited-unknown = 已退出
agent-status-failed = 失败：{ $error }
agent-activity-now = 刚刚
agent-activity-seconds = { $seconds } 秒前
agent-activity-finished = 已结束
agent-shared-tree-title = 共享工作区？
agent-shared-tree-warning = 另一个 Agent 会话仍在运行。两个会话共用当前仓库工作区，可能互相覆盖或产生冲突。
agent-close-title = 停止 Agent 会话？
agent-close-warning = 这会终止正在运行的 Agent 及其子进程。
agent-close-confirm = 停止会话
workspace-close-title = 停止正在运行的 Agent？
workspace-close-warning = 当前仍有 { $count } 个 Agent 会话运行中。关闭 Augur Git 将终止这些会话及其子进程。
workspace-tab-close-title = 停止此标签页中的 Agent？
workspace-tab-close-warning = 标签页“{ $tab }”仍有 { $count } 个 Agent 会话运行中。关闭它将终止这些会话及其子进程。
...
agent-default-profile-title = 默认 Agent 配置
