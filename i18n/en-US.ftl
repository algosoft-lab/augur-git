...
# ===== Agent lifecycle =====
workspace-close-title = Stop active Agent tests?
workspace-close-warning = { $count } Agent connectivity test(s) are still running. Closing Augur Git will terminate them.
...
agent-profiles-description = Built-in profiles use the CLI installed on your machine. Custom profiles and executable overrides are loaded from the application configuration.
agent-executable-title = CLI executable overrides (optional)
agent-launch-settings-title = Launch overrides
...
agent-profile-add = Add custom profile
...
agent-custom-profiles-title = Custom Agent profiles
...
agent-commit-note = The Agent is operating directly in this repository. Review its visible terminal and verify the refreshed Git state after it exits.
agent-commit-invalid-profile = The selected Agent profile is invalid: { $profile }
agent-commit-session-exited = Agent commit session exited (code: { $code }); repository state was refreshed.
