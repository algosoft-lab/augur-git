  ...
  return function(message)
    ...
    elseif type(result) ~= 'table' or not result.ok then
      ...
      pcall(augur.log, 'warn', 'extension file log write failed', {
        code = type(result) == 'table' and (result.code or 'log_write_failed')
          or 'log_write_failed',
...
local function run_repository(repo, settings, write_log)
  ...
  if not ready.ok then
    return failure(
      ready.code or 'repository_busy',
      ready.summary or 'repository stayed busy',
      steps
    )
  end
  ...
  if state.operation == 'merge' then
    ...
    if not result.ok then
      return failure(
        result.code or 'merge_recovery_failed',
        result.summary or 'merge recovery failed',
        steps
      )
    end
  elseif state.operation == 'rebase' then
    ...
    if not result.ok then
      return failure(
        result.code or 'rebase_recovery_failed',
        result.summary or 'rebase recovery failed',
        steps
      )
    end
  elseif state.operation ~= nil then
    return failure(
      ...
      'unsupported Git operation: ' .. tostring(state.operation),
      steps
    )
  end
  ...
  if state.dirty then
    ...
    local result = repo:agent_commit({
      hint = 'Commit the current worktree as one concise Conventional Commit.',
    })
    if not result.ok or not result.verified then
      return failure(
        result.code or 'agent_commit_unverified',
        result.summary or 'AI commit was not verified',
        steps
      )
    end
    state = repo:status()
    if state.dirty or state.conflicts then
      return failure(
        'worktree_not_clean',
        'AI commit did not leave a clean worktree',
        steps
      )
    end
  end
  ...
  if not pulled.ok then
    state = repo:status()
    if
      pulled.code == 'conflict'
      or state.operation == 'rebase'
      or state.conflicts
    then
      ...
      if not recovered.ok or not recovered.verified then
        return failure(
          recovered.code or 'rebase_recovery_unverified',
          recovered.summary or 'rebase recovery was not verified',
          steps
        )
      end
      state = repo:status()
      if state.operation ~= nil or state.conflicts then
        return failure(
          'rebase_state_unresolved',
          'rebase recovery left an unresolved state',
          steps
        )
      end
    else
      return failure(
        pulled.code or 'pull_failed',
        pulled.summary or 'pull --rebase failed',
        steps
      )
    end
  end
  ...
  if not pushed.ok then
    return failure(
      pushed.code or 'push_failed',
      pushed.summary or 'push failed',
      steps
    )
  end
...
local function sync(ctx)
  ...
  augur.notify(
    ...
    'Open tabs sync',
    string.format(
      '%d repositories synchronized, %d failed',
      #summary - failed,
      failed
    )
  )
  ...
  elseif failed > 0 then
    summary_text =
      string.format('%d of %d repositories failed', failed, #summary)
  else
